...
let extracted_earth = document.querySelector("#extracted-earth");
let extracted_moon = document.querySelector("#extracted-moon");
...
capture_btn.addEventListener("click", async function () {
  ...
  if (count == 1) {
    extracted_earth.setAttribute("src", capture_b64);
  } else {
    extracted_moon.setAttribute("src", capture_b64);
  }
  ...
  fetch("https://10.13.92.89:5000/removebg", {
    ...
    .then((d) => {
      ...
      if (count == 1) {
        extracted_earth.setAttribute("src", bg_rmv);
        count += 1;
      } else {
        extracted_moon.setAttribute("src", bg_rmv);
      }
    });
});
